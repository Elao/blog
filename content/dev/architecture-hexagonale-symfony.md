...
On va commencé cet article par une petite présentation de l'**architecture héxagonale**. Egalement appelé *Ports & Adapters*, 
...
Le grand principe de l'architecture hexagonale est la **séparation entre le code métier et l'infrastructure**. Le but est de rendre votre code métier **agnostique** de l'architecture sur laquelle votre application sera exécuté. Pour celà vous allez massivement utilise le **design pattern adapter** et l'**inversion de dépendance**.

La forme hexagonale — qui aurait tout aussi bien pu être octogonale ou pentagonale — est la pour mettre en évidence les différentes facette par laquel votre application communique avec l'exterieur via des adateurs.

L'architecture c'est tout l'environnement nécessaire à votre application sans faire parti du coeur métier de l'application. Tout ou partie de l'infrastructure peut être remplacer sans impacter votre métier. Cela comprend — entre autres — la persistence (base de données), le système de fichier, le cache, l'applicatif externe (API, binaires, ...), les bibliothèques, 
...
L'idée est que chaques couches peut utiliser une couche inférieure mais jamais une couche supérieur, ou en tout cas pas directement.
...
Les **événements** et les **exceptions** peuvent être lancés dans une couche inférieure et traités une couche supérieur. Quand au design pattern **adapter**, il permet de définir une interface du service donc vous avez besoin mais situé dans une couche suppérieur. L'adapteur correspondant sera lui implémenté dans ladite couche et l'injection de dépendances permettra d'assembler le tout en conservant le principe de séparation des couches.

Cette séparation en couches n'est pas indispensable à l'artchitecture héxagonale mais offre un cadre strict permettant de bien séparer votre votre code applicatif de votre infrastructure ainsi que les différentes parties de votre code. Une version simplifier pourrait se contenter de séparer Domain/Application de Infrastructure/UI.

## Qu'est ce qu'on met dedans ?

Dans la couche **Domain** j'ai tendances à mettre le coeur métier de mon code. Sans être exhausif, cela comprend mes entités, tout ce qui concerne les règles métiers (pour lesquel vous pouvez utiliser le pattern spécificaiton), les événéments et exceptions métier.

Dans la couche **Application** je place tout mon code applicatif. Généralement cela ce traduit par des *commands* et des *queries* (cf CQRS et CommandBus). Cette couche ce situant au dessus de la couche Domain, je pourrais utilisé tout ce qui s'y trouve. Si j'ai besoin de faire appel à des composants de l'infrastructure tel que la persistance, une API ou une library, je créérai des interfaces pour chacun de ces composants.

La couche **Infrastructure** contient majoritairement tout les implementation des adapteurs décrit dans les interfaces des couches inférieurs ainsi que tout les services nécessaires pour faire communiquer mon application avec mon infrastructure.
 
Enfin la couche **Ui** est une couche un peu particulière. Comme on peut le voir sur le schéma au début de l'article, elle occupe une facette de l'hexagone et n'entoure pas les autres couches. Il s'agit d'un sorte de couche adapteur géant qui permet à l'utilisateur de communiquer avec votre application. Elle contient donc tout ce qui touche à l'interface utilisateur comme les controlleurs, les templates de vue, les formulaires, ... 
...
Certes cette architectures est plus complexe, fait écrire un peu plus de code et demande un peu plus de reflextion qu'une artchitecture "classique" mais offre tout de même plusieurs avantage de taille.

1. Le code (en particulier le code métier) est beaucoup plus facile à tester unitairement. Il s'agit de pur PHP, débarrasser de toute relation à votre framework ou votre architecture. Toutes les dependences exterieur à votre métier sont des interfaces que vous pouvez facilement mocké ou implémenter pour vos tests. Votre code métier 100% testable unitairement.

2. Votre code métier et votre infrastructure étant complétement découplés, vous pouvez aisémenent faire évoluer votre infrastructure (changement de techno, monter de version, ...) sans jamais impacter votre code métier.

3. Vous pouvez décliner simplement application — par exemple en version CLI ou API — en conservant votre code metier et en changeant seulement les couches supérieures.

4. Les différentes partie de votre application étant bien découplé, vous pouvez simplement répartir leur développement sur plusieurs équipes.

5. Un code metier plus stable et pérenne.

6. En début de projet, vous pouvez mettre en place une architecture simple (persistence en mémoire/fichier, api mocké, ...) afin de vous concentrer sur la valeur ajouté : les fonctionnalités métier.
...
Pour la validation, nous allons également utiliser des fichiers [`yml` ou `xml`](https://symfony.com/doc/current/validation.html#the-basics-of-validation). A partir de la version 3.3, Symfony permet d'indiquer dans sa config les répertoires qui contiennent des fichiers de validation :
...
Deuxièmement, **vos controlleurs ne doivent pas contenir de logique metier** qui doit être restreint à vos seules couches Domain et Application. Vous devez uniquement faire appel à vos code métiers. De fait, vos controllers sont sensé être relativement court.
...
Enfin, **utilisez l'injection de dépendance** de Symfony pour injecter vos adapteur dans votre code métier.
